...
def main():
    ...
    if page == 'Homepage':
        ...
        sns.pairplot(df, vars=['magnesium', 'flavanoids',
                               'nonflavanoid_phenols', 'proline'], hue='alcohol')
        st.pyplot()
    else:
        st.title('Modelling')
        ...
        st.text(model.predict(
            df.drop(['alcohol'], axis=1).loc[row_number].values.reshape(1, -1))[0])
